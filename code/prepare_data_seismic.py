...
def get_img_paths(dir_path, wildcard='*.npy'):
...
#     x = int((w_end - w_start)/2)
    
    
#     w_start_small = x-int(size/4)
#     w_end_small = x+int(size/4)
    
#     return ai[h_start:h_end, w_start:w_end], seis[h_start:h_end, w_start_small:w_end_small]
...
def random_crop_same(ai,seis, size):
    h, w, c  = ai.shape
    ...
    return ai[h_start:h_end, w_start:w_end], seis[h_start:h_end, w_start:w_end]
